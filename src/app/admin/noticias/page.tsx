      ...
      <section className="border border-[#d8c38b] bg-white/76 p-6 shadow-[0_18px_50px_rgba(23,16,6,.08)]">
        ...
        <form action="/api/admin/posts" method="post" className="mt-8 grid gap-5">
          ...
          <label className="grid gap-2">
            ...
            <input name="titulo" required defaultValue={editingPost?.titulo} className="border border-[#d8c38b] bg-[#fffaf0] px-4 py-3 outline-none focus:border-[#8b2f2b]" placeholder="Ex.: COMIEADEPA realiza encontro regional..." />
          </label>

          <label className="grid gap-2">
            <span className="text-sm font-black uppercase tracking-[0.12em] text-[#5a472c]">Slug</span>
            <input name="slug" defaultValue={editingPost?.slug} className="border border-[#d8c38b] bg-[#fffaf0] px-4 py-3 outline-none focus:border-[#8b2f2b]" placeholder="gerado pelo título se ficar vazio" />
          </label>

          <div className="grid gap-5 md:grid-cols-3">
            <label className="grid gap-2">
              <span className="text-sm font-black uppercase tracking-[0.12em] text-[#5a472c]">Departamento</span>
              <select name="departamento_id" defaultValue={editingPost?.departamento_id ?? ""} className="border border-[#d8c38b] bg-[#fffaf0] px-4 py-3 outline-none focus:border-[#8b2f2b]">
            ...
            <label className="grid gap-2">
              ...
              <select name="categoria_id" defaultValue={editingPost?.categoria_id ?? ""} className="border border-[#d8c38b] bg-[#fffaf0] px-4 py-3 outline-none focus:border-[#8b2f2b]">
            ...
            <label className="grid gap-2">
              ...
              <select name="status" defaultValue={formatPostStatusLabel(editingPost?.status)} className="border border-[#d8c38b] bg-[#fffaf0] px-4 py-3 outline-none focus:border-[#8b2f2b]">
          ...
          <label className="grid gap-2">
            ...
            <textarea name="resumo" defaultValue={editingPost?.resumo ?? ""} className="min-h-28 border border-[#d8c38b] bg-[#fffaf0] px-4 py-3 outline-none focus:border-[#8b2f2b]" placeholder="Resumo para cards, SEO e chamadas da home." />
          </label>

          <RichTextField name="conteudo" label="Conteúdo" defaultValue={editingPost?.conteudo} placeholder="Texto completo da notícia, comunicado ou cobertura institucional." />

          <div className="grid gap-5 md:grid-cols-2">
            <MediaUrlField name="capa_url" label="URL da capa" defaultValue={editingPost?.capa_url} assets={mediaAssets} helper="Use uma imagem horizontal para melhor resultado nos cards e compartilhamentos." />
            <label className="grid gap-2">
              ...
              <span className="flex items-center gap-2 border border-[#d8c38b] bg-[#fffaf0] px-4 py-3">
                ...
                <input name="publicado_em" type="datetime-local" defaultValue={formatDateTimeLocal(editingPost?.publicado_em)} className="w-full bg-transparent outline-none" />
              </span>
            </label>
          </div>

          <label className="flex items-center gap-3 border border-[#d8c38b] bg-[#f7efd6] p-4 font-semibold text-[#342411]">
            <input name="destaque_home" type="checkbox" defaultChecked={editingPost?.destaque_home} className="h-5 w-5 accent-[#8b2f2b]" />
